...
This contract has a secondary merkle tree the 'dual tree' that takes the root of the 'value tree' as input, as a perpetual
input so previous proofs can always be verified against older versions of the value tree, keeping it stateless and ZK friendly.
...
contract EmpheralDualMerkleTreeKeccak is Ownable {
  ...
  uint32 public ROOT_HISTORY_SIZE = 60;
  ...
  function setHistorySize(uint32 _historySize) public onlyOwner {
    require(_historySize > ROOT_HISTORY_SIZE, "History size must be greater than current history size");
    ROOT_HISTORY_SIZE = _historySize;
  ...
  function findDivergenceLevelForNext(uint256 a) public view returns (uint256) {
    ...
    */
    ...
    bytes32[] calldata previousDualLeafPath) public onlyOwner returns (uint32 index)  {
      ...
      require(previousDualLeafPath.length == levels, "Dual value path length must be equal to levels");
      bytes32 newPathHash;
      bytes32 newDualPathHash;
...
//now do the same for the dual tree
    for (uint32 i = 0; i < levels; i++) {
        ...
        //If we hit divergence level we use the current hashed value
        if(i == divergenceLevel){         
          newDualPathHash = currentDualLeaf;
        }else{
          newDualPathHash = previousDualLeafPath[i];
        }
        
        //Normally hash the merkle path with a left right pos
        if (((currentIndex & (1<<i)) >> i) == 0) {         
           currentDualLeaf = _efficientHash(currentDualLeaf, previousDualLeafPath[i]);     
        } else {
          currentDualLeaf = _efficientHash(previousDualLeafPath[i], currentDualLeaf);
          //if the current level is before the divergence level we must use a null value
          //for hashing the new path.
          if(i<divergenceLevel){
            newDualPathHash = zeros(i);
          }
        }
        //We do normal left right hashing for the new path,
        //where the newDualPathHash is the altered version of the old path.
        newDualLeafHash = (((newIndex & (1 << i)) >> i) == 1) 
            ? _efficientHash(newDualPathHash, newDualLeafHash)
            : _efficientHash(newDualLeafHash, newDualPathHash);

...
function extractBytes32List(bytes memory data) public pure returns (bytes32[] memory) {
   ...
   * @dev publicSignals[4] is the blockHash of the previous block at insertion time
  ...
  function verify(bytes calldata proof, bytes32[] calldata publicSignals) external pure returns (bool) {
    ...
    bytes32 currentLeafValue = _efficientHash(publicSignals[2], _efficientHash(publicSignals[1], publicSignals[4]));

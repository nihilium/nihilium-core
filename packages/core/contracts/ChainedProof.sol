...
//We then call chain_proof_verify
    ...
    function dryrun_validate_data_root(ProvingState calldata state, 
        ...
        uint256 output_signal_index) external view returns (ProvingState memory) {
            ProvingState memory new_state = state;
...
//Or even the same datastream itself
    function dryrun_chain_pass_signal(ProvingState calldata state, 
        ...
        uint256 output_proof_index, 
        uint256[2] calldata output_indexes) external pure returns (ProvingState memory) {
        ProvingState memory new_state = state;
       ...
       // assert(new_state.prepared_proof.length > 0);
        ...
        for (uint256 i = 0; i < public_input_indexes[1]; i++) {
            new_state.prepared_public_inputs[public_input_indexes[0] + i] = new_state.outputs[output_proof_index][output_indexes[0] + i];
            new_state.current_hash = keccak256(abi.encodePacked(new_state.current_hash, public_input_indexes[0] + i, output_proof_index,output_indexes[0] + i));
        }
        return new_state;
    }

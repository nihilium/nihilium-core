 ...
 */
//@dev deprecated, use EmpheralMerkleTreeKeccak instead
contract DualMerkleTree is Ownable {
  ...
  mapping(uint256 => uint256) public filledValueSubtrees;
  mapping(uint256 => uint256) public filledmerkleRootSubtrees;
  ...
  constructor(
    ...
    levels = _levels;
    //hasher = _hasher;
    treeHasher = _treeHasher;
    maxValue = (uint32(2)**18)-1;
    for (uint32 i = 0; i < _levels; i++) {
      filledValueSubtrees[i] = zeros(i);
      filledmerkleRootSubtrees[i] = zeros(i);
    }

    valueRoots[0] = zeros(_levels);
    merkleRoots[0] = zeros(_levels);
...
//   }
  ...
  function insert(uint256 _leaf, address _origin) public onlyOwner returns (uint32 index)  {
    ...
    for (uint32 i = 0; i < levels; i++) {
      if (currentIndex % 2 == 0) {
        ...
        right = zeros(i);
        filledValueSubtrees[i] = currentLevelHash;
      } else {
        left = filledValueSubtrees[i];

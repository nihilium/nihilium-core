...
import { generateRandom248BitNumber, portableArgon2, toBytesLE, fromBytesLE, bigInt2Buffer, bufferToBigInt, } from "./tools";
...
export function FDTEncrypt(message, pubKeys, threshold) {
    ...
    combinations.forEach((combination) => {
        ...
        const sorted = [...combination].sort((a, b) => a.toHex().localeCompare(b.toHex()));
...
export function FDTDecrypt(privateKeys, ciphertextMap, ephemeralKeys) {
    ...
    const ephemeralPoints = ephemeralKeys.map((ek) => {
...
// --- Helpers ---
function combinePublicKeys(pubKeys) {
...
function generateCombinations(pubKeys, threshold) {
    const n = pubKeys.length;
    const k = threshold;
    if (k > n)
        return [];
    if (k === 1)
        return pubKeys.map((pk) => [pk]);
    if (k === 0)
        return [[]];
    const combinations = [];

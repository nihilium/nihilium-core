...
const AND_GATES_PER_EC_MULT = 800000000; // ~800M gates
 ...
 */
export function estimateRounds(difficulty, threshold, poolSize) {
    if (difficulty < 1)
 ...
 */
export function difficultyTable(difficulty, configs) {
    console.log(`\nDifficulty level: ${difficulty} (reference: ${difficulty} rounds at k=${REFERENCE_K})\n`);
    console.log("Config".padEnd(10), "Rounds".padEnd(8), "C(n,k)".padEnd(10), "MPC EC mults".padEnd(14), "MPC Comms".padEnd(12), "MPC Transfer".padEnd(15), "Seal time".padEnd(12), "Unseal time");
    console.log("-".repeat(95));
    for (const { k, n } of configs) {
        const est = estimateRounds(difficulty, k, n);
        const cnk = binomial(n, k);
        console.log(`${k}-of-${n}`.padEnd(10), `${est.rounds}`.padEnd(8), `${cnk}`.padEnd(10), `${est.mpcEcMults}`.padEnd(14), `${est.mpcCommunicationGB} GB`.padEnd(12), `${est.mpcTransferTimeMinutes1Gbps} min`.padEnd(15), `${est.clientSealingTimeFormatted}`.padEnd(12), `${est.unsealTimeMs} ms`);
    }
}
// --- Helpers ---

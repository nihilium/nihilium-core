...
function challenge(B1, P1, B2, P2, A1, A2) {
    const preimage = concatBytes(DOMAIN, B1.toRawBytes(), P1.toRawBytes(), B2.toRawBytes(), P2.toRawBytes(), A1.toRawBytes(), A2.toRawBytes());
 ...
 * and B2 = ek (so P2 = D).
 */
export function proveDLEQ(x, B1, B2) {
    const xr = mod(x);
 ...
 */
export function verifyDLEQ(B1, P1, B2, P2, proof) {
    try {
        const { c, z } = proof;
        if (c <= 0n || c >= L || z <= 0n || z >= L)
            return false;
 ...
 */
export function provePartialDecryption(sk, ek) {
 ...
 */
export function attributePartialDecryption(pk, ek, partial) {
 ...
 */
export function attributeCombinedPartial(candidatePubKeys, ek, partial, opts = {}) {
    ...
    for (let mask = 1; mask < 1 << n; mask++) {
        ...
        for (let i = 0; i < n; i++)
            if (mask & (1 << i))
                idx.push(i);
        if (idx.length < minSize)
            continue;
        let sum = candidatePubKeys[idx[0]];
        for (let j = 1; j < idx.length; j++)
            sum = sum.add(candidatePubKeys[idx[j]]);
        if (attributePartialDecryption(sum, ek, partial))
            return idx;
    }
    return null;
}
const scalarToHex = (s) => s.toString(16);
const hexToScalar = (h) => BigInt("0x" + h);
...
export function deserializePartial(s) {
    return {

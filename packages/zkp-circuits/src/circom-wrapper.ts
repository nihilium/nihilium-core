...
let fs: any, path: any, https: any, http: any, URL: any;
...
export class WrappedCircomCircuit implements CircuitWrapper<any> {
  ...
  private static cacheDir = typeof window === 'undefined' && path ? path.join(process.cwd(), '.ipfs-cache') : '';
   ...
   */
  private static async downloadIPFSFile(url: string, filename: string): Promise<string> {
   ...
   */
  private static async resolveIPFSPath(ipfsUrl: string, filename: string): Promise<string> {
  ...
  async init(): Promise<void> {
    ...
    if (isIPFSUrl(this.config.wasmPath)) {
      ...
      this.resolvedWasmPath = await WrappedCircomCircuit.resolveIPFSPath(
    ...
    if (isIPFSUrl(this.config.zkeyPath)) {
      ...
      this.resolvedZkeyPath = await WrappedCircomCircuit.resolveIPFSPath(
    ...
    if (this.config.vkeyPath) {
      if (isIPFSUrl(this.config.vkeyPath)) {
        ...
        this.resolvedVkeyPath = await WrappedCircomCircuit.resolveIPFSPath(

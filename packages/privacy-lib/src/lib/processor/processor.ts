...
export class Processor {
  ...
  async process_unseal_request(request: SingleUnsealRequest): Promise<SingleSealUnsealRequestResponse> {
    ...
    //
    var cyphertexts = request.cyphertexts.map(signal => BigInt(signal));
    var ephemeral_keys = request.empheral_keys.map(signal => BigInt(signal));
    var public_key = request.public_signals[0].slice(5, 7).map(signal => BigInt(signal));

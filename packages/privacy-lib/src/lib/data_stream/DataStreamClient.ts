...
export class DataStreamClient implements IDataStream {
    ...
    async getProof(value: HexString): Promise<[ProofPath, ProofPath, number, number, number]> {
        ...
        return [result.globalProof, result.localProof, parseInt(result.timestamp), parseInt(result.globalIndex), parseInt(result.localIndex)]
    }   
    ...
    async getLatestGlobalLeafProof(): Promise<[ProofPath, string, number, number]> {
        ...
        result.globalProof.pathElements = result.globalProof.pathElements.map((element: any) => BigInt(element))
        result.globalProof.pathIndices = result.globalProof.pathIndices.map((element: any) => BigInt(element))
        return [result.globalProof as ProofPath, result.leaf as string, parseInt(result.timestamp), parseInt(result.globalIndex)]
    }
